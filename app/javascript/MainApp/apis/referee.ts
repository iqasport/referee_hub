...
const formatRefereeResponse = (
  ...
  const locations = included
    .filter(
      (record: Included) =>
        record.type === "refereeLocation" &&
        relationships.refereeLocations.data.find((loc) => loc.id === record.id)
    )
    .map(mapAttributes);
  const certifications = included
    .filter(
      (record: Included) =>
        ...
        relationships.certifications.data.find((cert) => cert.id === record.id)
    )
    .map(mapAttributes);
  const testAttempts = included
    .filter(
      (record: Included) =>
        record.type === "testAttempt" &&
        relationships.testAttempts.data.find((attempt) => attempt.id === record.id)
    )
    .map(mapAttributes);
  const testResults = included
    .filter(
      (record: Included) =>
        record.type === "testResult" &&
        relationships.testResults.data.find((result) => result.id === record.id)
    )
    .map(mapAttributes);
  const ngbs = included
    .filter(
      (record: Included) =>
        record.type === "nationalGoverningBody" &&
        relationships.nationalGoverningBodies.data.find((ngb) => ngb.id === record.id)
    )
    .map(
      (record: Included): IdAttributes => ({
        ...record.attributes,
        nationalGoverningBodyId: parseInt(record.id, 10),
        id: record.id,
      })
    );
  const teamsData = included
    .filter(
      (record: Included) =>
        record.type === "team" && relationships.teams.data.find((team) => team.id === record.id)
    )
    .map(
      (record: Included): IdAttributes => ({
        ...record.attributes,
        teamId: parseInt(record.id, 10),
        id: record.id,
      })
    );
  const teams = included
    .filter(
      (record: Included) =>
        record.type === "refereeTeam" &&
        relationships.refereeTeams.data.find((refTeam) => refTeam.id === record.id)
    )
    .map(
      (refereeTeam: Included): IdAttributes => {
        const team = teamsData.find(
          (teamData: IdAttributes) => teamData.teamId === refereeTeam.attributes.teamId
        );
