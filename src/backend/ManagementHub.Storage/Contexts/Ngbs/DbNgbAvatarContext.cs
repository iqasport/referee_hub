...
public class DbNgbAvatarContextFactory
{
	...
	{
		this.logger.LogInformation(-0x22a8f500, "Getting avatar context for NGB ({ngbId}).", ngbId);
		...
		{
			this.logger.LogError(-0x22a8f4ff, ex, "Error while getting avatar URI for NGB ({ngbId}).", ngbId);
			throw;
		}
	}
}
