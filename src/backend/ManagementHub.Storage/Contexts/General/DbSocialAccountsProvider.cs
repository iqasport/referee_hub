...
public class DbSocialAccountsProvider : ISocialAccountsProvider
{
	...
	{
		return this.dbContext.SocialAccounts.Where(sa => sa.OwnableType == "NationalGoverningBody")
			.Join(this.dbContext.NationalGoverningBodies.WithConstraint(ngbConstraint), sa => sa.OwnableId, ngb => ngb.Id, (sa, ngb) => new { ngb.CountryCode, sa.Url, sa.AccountType })
	...
	{
		var teams = this.dbContext.Teams.Join(this.dbContext.NationalGoverningBodies.WithConstraint(ngbConstraint), t => t.NationalGoverningBodyId, n => n.Id, (t, _) => t);
		return this.dbContext.SocialAccounts.Where(sa => sa.OwnableType == "Team")
