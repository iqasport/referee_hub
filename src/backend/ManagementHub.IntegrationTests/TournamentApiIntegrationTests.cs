...
public class TournamentApiIntegrationTests : IClassFixture<TestWebApplicationFactory>
{
	...
	[Fact]
	public async Task FantasyTournament_WithBothRegistrationTypes_ShouldStoreAndReturnCorrectly()
	{
		await AuthenticationHelper.AuthenticateAsAsync(this._client, "referee@example.com", "password");

		// Create a Fantasy tournament with both registration types enabled
		var createModel = new TournamentModel
		{
			Name = "Fantasy Both Types Tournament",
			Description = "Fantasy tournament allowing both teams and individual players",
			StartDate = DateOnly.FromDateTime(DateTime.UtcNow.AddDays(30)),
			EndDate = DateOnly.FromDateTime(DateTime.UtcNow.AddDays(32)),
		...
		var createResponse = await this._client.PostAsJsonAsync("/api/v2/tournaments", createModel);
		createResponse.StatusCode.Should().Be(HttpStatusCode.OK, "creating Fantasy tournament with both types should succeed");

		var tournamentIdResponse = await createResponse.Content.ReadFromJsonAsync<TournamentIdResponseDto>();
		var tournamentId = tournamentIdResponse!.Id;

		// Verify the fields are returned correctly
		var getResponse = await this._client.GetAsync($"/api/v2/tournaments/{tournamentId}");
		getResponse.StatusCode.Should().Be(HttpStatusCode.OK);

		var tournament = await getResponse.Content.ReadFromJsonAsync<TournamentViewModelDto>();
		tournament.Should().NotBeNull();
		tournament!.AllowsTeamRegistration.Should().BeTrue("team registration should be enabled");
		tournament.AllowsIndividualRegistration.Should().BeTrue("individual registration should be enabled");
	}

	[Fact]
	public async Task FantasyTournament_WithNoRegistrationTypes_ShouldReturnBadRequest()
	{
		await AuthenticationHelper.AuthenticateAsAsync(this._client, "referee@example.com", "password");

		// Try to create a Fantasy tournament with neither registration type enabled
		var createModel = new TournamentModel
		{
		...
		createResponse.StatusCode.Should().Be(HttpStatusCode.BadRequest,
			"Fantasy tournament with no registration types should be rejected");

		var errorContent = await createResponse.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>();
		errorContent.GetProperty("error").GetString().Should().Contain("at least one registration type",
			"error should explain that at least one type is required");
	}

	[Fact]
	public async Task FantasyTournament_IndividualPlayerInvite_ShouldSucceed()
	{
		await AuthenticationHelper.AuthenticateAsAsync(this._client, "referee@example.com", "password");

		// Create a Fantasy tournament that allows individual registration
		var createModel = new TournamentModel
		{
			Name = "Fantasy Individual Test Tournament",
			Description = "Fantasy tournament for individual player invite test",
			StartDate = DateOnly.FromDateTime(DateTime.UtcNow.AddDays(30)),
			EndDate = DateOnly.FromDateTime(DateTime.UtcNow.AddDays(32)),
			Type = TournamentType.Fantasy,
			Country = "USA",
		...
		var createResponse = await this._client.PostAsJsonAsync("/api/v2/tournaments", createModel);
		createResponse.StatusCode.Should().Be(HttpStatusCode.OK);
		var tournamentId = (await createResponse.Content.ReadFromJsonAsync<TournamentIdResponseDto>())!.Id;

		// Switch to team_manager user and invite themselves as an individual player
		await AuthenticationHelper.AuthenticateAsAsync(this._client, "team_manager@example.com", "password");

		// Get the current user's ID
		var meResponse = await this._client.GetAsync("/api/v2/users/me");
		meResponse.StatusCode.Should().Be(HttpStatusCode.OK);
		var me = await meResponse.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>();
		var playerId = me.GetProperty("userId").GetString();
		playerId.Should().NotBeNullOrEmpty();

		// Create an individual invite
		var inviteResponse = await this._client.PostAsJsonAsync($"/api/v2/tournaments/{tournamentId}/invites", new
		{
			participantType = "player",
			participantId = playerId,
		});
		inviteResponse.StatusCode.Should().Be(HttpStatusCode.Created,
			"individual player invite should succeed for Fantasy tournament with individual registration enabled");
	}
}
